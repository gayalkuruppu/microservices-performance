...
for s in range(0, 121, 24):
    ...
    plt.plot(data['Users'].iloc[s:s+4], data['Throughput'].iloc[s:s+4], label='One Service', marker='o')
    plt.plot(data['Users'].iloc[s+8:s+12], data['Throughput'].iloc[s+8:s+12], label='Two Services', marker='o')
    plt.plot(data['Users'].iloc[s+16:s+20], data['Throughput'].iloc[s+16:s+20], label='Three Services', marker='o')

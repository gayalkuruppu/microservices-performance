...
#CSV file
csv_file_records = []
headers = ['Num Service', 'Users', 'Throughput','Avg_Latency', 'percentile_10', 'percentile_50', 'percentile_90', 'percentile_99', 'percentile_99.9','percentile_99.99','Std','Tail Index']
...
jtl_location="/media/jayathma/OS/Jayathma_R/Results/DB-Chain/DBC"
summary_location="/media/jayathma/PartitionE/Jayathma_R/TransformedCSV/ChainDB-C"
output_csv_location="/media/jayathma/PartitionE/Jayathma_R/TransformedCSV/ChainDB-C/chain.csv"
...
for num in num_services:
	...
	for user in concurrent_users:
		...
		jtl_file_name = jtl_location+"/"+str(num)+"_chaindbc/jtls/"+str(user)+"_users/results-measurement.jtl"
		
		summary_file_name = summary_location+"/"+str(num)+"_Chain.csv"
		...
		row = [num,user,TPS,avg_Latency,percentile_10,percentile_50,percentile_90,percentile_99,percentile_99_9,percentile_99_99,std,tail_index]
		csv_file_records.append(row)
		
	print ("Completed Generating CSV files for "+ str(user) + " users")
	
print ("Completed for " + str(num) + " db")

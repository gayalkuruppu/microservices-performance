...
message_sizes=(521 7919 10007 100003)
...
target_script=/home/fct/Project/Builds/Ballerina/async-factorial-chaining-three/start.sh
target_uptime_script=/home/fct/Project/Builds/Ballerina/async-factorial-chaining-three/uptime.sh
target_uptime_path=/home/fct/Project/Builds/Ballerina/async-factorial-chaining-three/uptime_dir

###Machine A
host1_ip=172.16.53.102
host1_port=8080
host1_username_ip=fct@172.16.53.102
host1_pwd=123
host1_machine_num=1

###Machine B
host2_ip=172.16.53.109
host2_port=8081
host2_username_ip=fct@172.16.53.109
host2_pwd=123
host2_machine_num=2

###Machine C
host3_ip=172.16.53.54
host3_port=8082
host3_username_ip=fct@172.16.53.54
host3_pwd=123
host3_machine_num=3
...
jtl_location=/home/fct/Projects/ballerina-0-981-1/Results/async-factorial-chaining-three/jtls
jmx_file=/home/fct/Projects/ballerina-0-981-1/Tests/async-factorial-chaining-three/Chaining_Three_Factorial_Test.jmx
uptime_path=/home/fct/Projects/ballerina-0-981-1/Results/async-factorial-chaining-three
	...
	do
		...
		do
			...
			do
				...
				response_code=$(curl -s -o /dev/null -w "%{http_code}" -X GET -d "hello" http://${host1_ip}:${host1_port}/prime/test)
			...
			do
				echo "Checking service"
				response_code=$(curl -s -o /dev/null -w "%{http_code}" -X GET -d "hello" http://${host2_ip}:${host2_port}/prime/test)
				if [ $response_code -eq 200 ]; then
			...
			do
				echo "Checking service"
				response_code=$(curl -s -o /dev/null -w "%{http_code}" -X GET -d "hello" http://${host3_ip}:${host3_port}/prime/test)
				if [ $response_code -eq 200 ]; then
			...
			echo "Begin test for ${u} users and ${size} "
			...
			echo "Completed Generating JTL files for ${u} users and ${size}"
		done

		echo "Completed tests for ${size} "
